...
export function renderCard(object, isFav) {
    const listingContainer = document.querySelector('#listContainer');
    const markup = `<article class="col-md-4">
                    <!-- card -->
                    <a href="#/item/${object.id}" class="card" id="${object.id}">
...
export function toggleFav(target, isFav){
    if(isFav){
       target.closest('.card__like').classList.add('card__like--active')
    } else{
        target.closest('.card__like').classList.remove('card__like--active')
    }
}